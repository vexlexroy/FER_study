...
public class Sphere extends Object{
  ...
  public boolean intersection(Ray ray) {
    ...
    Vector ray_to_cent =  new Vector(origin, this.getCenterPosition());
    double angle = Math.acos(direction.dotProduct(ray_to_cent)/(direction.getLength()*ray_to_cent.getLength()));
    if(angle >= Math.PI/2 ||  angle <= -Math.PI/2){ // kut zrake i centra kugle
      return false;
    }
    else{
      double d = ray_to_cent.getLength()*Math.sin(angle); // distanca od sredista kugle
      if (d > this.radius)
          return false;
